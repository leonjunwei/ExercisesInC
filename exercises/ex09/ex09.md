...
* Uhh... I'm really bad at ASCII art. What I think is happening is that the parent process spawns 3 new threads (probably in sequence).

* Then each child process runs child_code after a different amount of time and exits, and the parent calls wait (or join?) on each of them once they're done to kill the process
...
* My guess is something to do with how execle exits once it's done. It probably breaks the loop or something?
