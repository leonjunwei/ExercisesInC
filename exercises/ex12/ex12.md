...
* The final value of the counter is always correct, but the printouts from each child aren't always.
...
* After a few hundred children, occasionally the final value of the counters aren't correct.
...
* Around twenty thousand of them!
...
* real  0.020s, user 0.015s, sys 0.005s for counter_array.
* real  0.056s, user 0.036s, sys 0.041s for counter_mutex.
* Overhead is around 0.02 to 0.04 seconds, which is a lot for something that only takes between 0.02 and 0.06s to complete.
